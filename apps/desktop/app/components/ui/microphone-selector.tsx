  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button
          type="button"
          className={`${triggerButtonClassName} h-9 px-4 rounded-md bg-[#2A2B2E] text-[#979899] border border-[#3a3a3b] hover:border-[#979899] text-sm font-medium transition-all cursor-pointer`}
        >
          {triggerButtonText === 'Select Microphone' &&
              ...
              <div
                ...
                className={`p-6 rounded-md cursor-pointer transition-colors max-w-full overflow-hidden ${
                  tempSelectedMicrophone === mic.deviceId
                    ? 'bg-[#2A2B2E] border-2 border-[#3a3a3b]'
                    : 'bg-[#1C1E21] border-2 border-[#1C1E21] hover:bg-[#2A2B2E]'
                }`}
                onClick={() => handleMicrophoneSelect(mic.deviceId)}
                style={{ minWidth: 0 }}
              >
                <div
                  className="font-medium text-base truncate text-[#979899]"
                  style={{ maxWidth: '100%' }}
                >
                  {title}
                </div>
                {description && (
                  <div
                    className="text-sm text-[#979899] text-wrap mt-2"
                    style={{ maxWidth: '100%' }}
                  >
                    {description}
                  </div>
        ...
        <div className="flex justify-end px-8 pb-8 pt-6">
          <DialogClose asChild>
            <button
              ...
              className="w-32 h-9 rounded-md bg-[#2A2B2E] text-[#979899] border border-[#2A2B2E] hover:border-[#3a3a3b] text-sm font-medium transition-all cursor-pointer"
            >
              Save and close
            </button>
          </DialogClose>
