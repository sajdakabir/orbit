  ...
  return (
    <div className={`rounded-lg ${className}`}>
      {isEditing ? (
        <>
          {!hideTitle && (
            <div className="text-lg font-medium mb-6 text-center text-white">
              {editModeTitle}
            </div>
          )}
          <div
            className="flex justify-center items-center mb-4 w-full py-3 rounded-lg gap-2"
            style={{ minHeight }}
          >
          ...
          >
            {shortcutKeys.map((keyboardKey, index) => (
              <KeyboardKey
                key={index}
                keyboardKey={keyboardKey}
                className={`${isDisplayKeyPressed(String(keyboardKey), pressedKeys) ? 'bg-white border-2 border-white text-black' : 'bg-neutral-800 border-2 border-neutral-700'}`}
                style={{
                  width: `${keySize}px`,
                  height: `${keySize}px`,
                }}
