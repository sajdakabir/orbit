...
The Orbit transcription server provides gRPC-based speech-to-text services for the Orbit voice assistant application. This server handles audio transcription, user data management, and API authentication.
...
BLOB_STORAGE_BUCKET=orbit-audio-storage
...
- Auto-creates the `orbit-audio-storage` bucket
...
| `REQUIRE_AUTH`         | Yes      | `true`      | Authentication is always required           |
| `AUTH0_DOMAIN`         | Yes      | -           | Auth0 domain for authentication             |
| `AUTH0_AUDIENCE`       | Yes      | -           | Auth0 audience for authentication           |
...
- **Auto-bucket creation**: `orbit-audio-storage` created automatically
...
# Run migrations
docker compose exec orbit-grpc-server bun run db:migrate
```
...
```bash
# Authentication is required - configure Auth0 credentials
echo "REQUIRE_AUTH=true" >> .env
echo "AUTH0_DOMAIN=your-auth0-domain.auth0.com" >> .env
echo "AUTH0_AUDIENCE=http://localhost:3000" >> .env
```
...
The API is defined in `src/orbit.proto`. Key services:
...
The Orbit desktop app automatically connects to `localhost:3000`. Ensure the server is running before starting the desktop application.
...
- **Issues**: [GitHub Issues](https://github.com/sajdakabir/orbit/issues)
