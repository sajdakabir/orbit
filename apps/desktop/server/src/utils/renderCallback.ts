...
export function renderComposioCallbackPage(
  ...
  const title = isSuccess ? "You're all done!" : 'Connection Failed'
  const subtitle = isSuccess
    ? 'Your account has been connected successfully. You can now return to the app.'
  ...
  <head>
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .container { text-align: center; max-width: 420px; padding: 48px 32px; }
      .icon-circle {
      ...
      .icon-circle svg { width: 36px; height: 36px; color: ${iconColor}; }
      h1 { font-size: 24px; font-weight: 700; margin-bottom: 8px; }
      .subtitle { font-size: 15px; color: #979899; margin-bottom: 32px; line-height: 1.5; }
      .open-btn {
      ...
      .close-hint { margin-top: 16px; font-size: 13px; color: #525252; }
    </style>
  </head>
  <body>
    <div class="container">
      ...
      <a href="${deepLink}" class="open-btn">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
        </svg>
        Open App
      </a>
      <p class="close-hint">You can close this tab after opening the app.</p>
    </div>
    <script>
...
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Authentication Successful - Orbit</title>
    <style>
      * {
        margin: 0;
        padding: 0;
  ...
  <body>
    <div>
      ...
      <button class="button" onclick="openApp()">Open Orbit</button>
    </div>
