...
export function enhancePcm16(pcm: Buffer, sampleRate: number): Buffer {
  ...
  // Gentle high-pass filter (~100 Hz) - removes low-frequency rumble for better intelligibility
  const fc = 100
  ...
  // Peak normalize to ~-3 dBFS, cap max gain to ~+18 dB (handles very quiet speech)
  let peak = 1
  for (let i = 0; i < sampleCount; i++) {
  ...
  const gain = Math.min(rawGain, 8.0)
