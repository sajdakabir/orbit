...
export class TranscribeStreamHandler {
  async process(requests: AsyncIterable<AudioChunk>, context: HandlerContext) {
    ...
    try {
      ...
      const mode = getOrbitMode(context.requestHeader.get('mode'))
      ...
      const windowContext: OrbitContext = { windowTitle, appName, contextText }

      const detectedMode = mode || detectOrbitMode(transcript)
      ...
      if (detectedMode === OrbitMode.EDIT) {
        ...
        transcript = await llmProvider.adjustTranscript(
          userPromptPrefix + '\n' + userPrompt,
          {
            ...
            prompt: ORBIT_MODE_SYSTEM_PROMPT[detectedMode],
          },
        )
        console.log(
