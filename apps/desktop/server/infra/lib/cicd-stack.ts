...
export class GitHubOidcStack extends Stack {
  constructor(scope: Construct, id: string, props: GitHubOidcStackProps) {
    ...
    const principal = new OpenIdConnectPrincipal(oidc, {
      StringLike: {
        'token.actions.githubusercontent.com:sub': `repo:sajdakabir/orbit:*`,
        'token.actions.githubusercontent.com:aud': 'sts.amazonaws.com',
      },
    })

    // ─── create the CI/CD role ─────────────────────────────────────────────────
    const ciCdRole = new Role(this, 'OrbitGitHubCiCdRole', {
      assumedBy: principal,
      roleName: 'OrbitGitHubCiCdRole',
      description: 'GitHub Actions can assume this via OIDC',
    })

    // ─── create s3 bucket for releases ────────────────────────
    props.stages.forEach(stage => {
      const bucketName = `${stage}-${ORBIT_PREFIX.toLowerCase()}-releases`
      const bucket = new Bucket(this, `${stage}-OrbitReleasesBucket`, {
        bucketName,
    ...
    // ─── CloudFormation on any of our “Orbit*” stacks ──────────────────────────────
    const cfnArnPattern = `arn:aws:cloudformation:${this.region}:${this.account}:stack/${ORBIT_PREFIX}*/*`
    ciCdRole.addToPolicy(
      new PolicyStatement({
        effect: Effect.ALLOW,
        actions: [
