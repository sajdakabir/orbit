...
export class PlatformStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: PlatformStackProps) {
    ...
    const dbSecurityGroup = new SecurityGroup(this, 'OrbitDbSecurityGroup', {
      vpc: props.vpc,
      description: 'Allow ECS Fargate service to connect to Aurora',
      allowAllOutbound: true,
    })

    const dbCredentialsSecret = new Secret(this, 'OrbitDbCredentials', {
      secretName: `${stageName}/orbit-db/dbadmin`,
    ...
    const dbCluster = new DatabaseCluster(this, 'OrbitAuroraServerless', {
    ...
    this.serviceRepo = new Repository(this, 'OrbitServiceRepo', {
    ...
    this.blobStorageBucket = new Bucket(this, 'OrbitBlobStorage', {
      bucketName: `${stageName}-${this.account}-${this.region}-orbit-blob-storage`,
      removalPolicy: isDev(stageName)
        ? RemovalPolicy.DESTROY
        : RemovalPolicy.RETAIN,
    ...
    const firehoseRole = new Role(this, 'OrbitFirehoseRole', {
      assumedBy: new ServicePrincipal('firehose.amazonaws.com'),
      roleName: `${stageName}-OrbitFirehoseRole`,
    })

    // Cognito resources for OpenSearch Dashboards authentication
    const userPool = new UserPool(this, 'OrbitOsUserPool', {
      userPoolName: `${stageName}-orbit-os-userpool`,
      selfSignUpEnabled: true,
      signInAliases: { email: true, username: true },
      accountRecovery: undefined,
    })
    new UserPoolClient(this, 'OrbitOsUserPoolClient', {
      userPool,
      userPoolClientName: `${stageName}-orbit-os-client`,
      generateSecret: false,
      authFlows: { userSrp: true, userPassword: true },
    })
    new UserPoolDomain(this, 'OrbitOsUserPoolDomain', {
      userPool,
      cognitoDomain: {
        domainPrefix: `${stageName}-${this.account}-orbit-os`.toLowerCase(),
      },
    })
    const identityPool = new CfnIdentityPool(this, 'OrbitOsIdentityPool', {
      allowUnauthenticatedIdentities: true,
      identityPoolName: `${stageName}-orbit-os-identitypool`,
      // Leave providers empty so OpenSearch can register its own App Client
    })
    const authenticatedRole = new Role(this, 'OrbitOsCognitoAuthenticatedRole', {
    ...
    const unauthenticatedRole = new Role(
      this,
      'OrbitOsCognitoUnauthenticatedRole',
      {
        assumedBy: new FederatedPrincipal(
          'cognito-identity.amazonaws.com',
          {
    ...
    new CfnIdentityPoolRoleAttachment(this, 'OrbitOsIdentityPoolRoleAttachment', {
    ...
    const cognitoAccessRole = new Role(this, 'OrbitOsCognitoAccessRole', {
    ...
    const domain = new Domain(this, 'OrbitLogsDomain', {
      domainName: `${stageName}-orbit-logs`,
    ...
    domain.addAccessPolicies(
      new PolicyStatement({
        ...
        conditions: {
          ...
          ArnLike: {
            'aws:SourceArn': [
              `arn:aws:firehose:${this.region}:${this.account}:deliverystream/${stageName}-orbit-client-logs`,
              `arn:aws:firehose:${this.region}:${this.account}:deliverystream/${stageName}-orbit-server-logs`,
            ],
          },
        },
      }),
