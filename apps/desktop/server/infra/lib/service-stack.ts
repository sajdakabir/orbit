...
export class ServiceStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ServiceStackProps) {
    ...
    const groqApiKeySecret = Secret.fromSecretNameV2(
      this,
      'GroqApiKey',
      `${stageName}/orbit/groq-api-key`,
    )

    const cerebrasApiKeySecret = Secret.fromSecretNameV2(
      this,
      'CerebrasApiKey',
      `${stageName}/orbit/cerebras-api-key`,
    )

    const stripeSecretKeySecret = Secret.fromSecretNameV2(
      this,
      'StripeSecretKey',
      `${stageName}/orbit/stripe-secret-key`,
    )

    const stripeWebhookSecret = Secret.fromSecretNameV2(
      this,
      'StripeWebhookSecret',
      `${stageName}/orbit/stripe-webhook`,
    )

    // Setup domain and certificate
    const zone = HostedZone.fromLookup(this, 'HostedZone', {
      domainName: 'orbit-api.com',
    })

    const domainName = `${stageName}.orbit-api.com`
    ...
    // Create ECS cluster
    const cluster = new Cluster(this, 'OrbitEcsCluster', {
      vpc: props.vpc,
      clusterName: `${stageName}-${CLUSTER_NAME}`,
    })

    // Create S3 buckets
    const logBucket = new Bucket(this, 'OrbitAlbLogsBucket', {
      bucketName: `${stageName}-${this.account}-${this.region}-orbit-alb-logs`,
    ...
    const firehoseBackupBucket = new Bucket(this, 'OrbitFirehoseBackupBucket', {
      bucketName: `${stageName}-${this.account}-${this.region}-orbit-firehose-bucket`,
      removalPolicy: isDev(stageName)
        ? RemovalPolicy.DESTROY
        : RemovalPolicy.RETAIN,
      blockPublicAccess: BlockPublicAccess.BLOCK_ALL,
    ...
    scalableTarget.scaleOnCpuUtilization('OrbitServerCpuScalingPolicy', {
    ...
    alb.logAccessLogs(logBucket, 'orbit-alb-access-logs')
    ...
    const firehoseRole = IamRole.fromRoleArn(
      this,
      'OrbitFirehoseRoleImported',
      `arn:aws:iam::${this.account}:role/${stageName}-OrbitFirehoseRole`,
      { mutable: true },
    )
