...
export function createTimingInfrastructure(
  ...
  const timingBucket = new Bucket(scope, 'OrbitTimingDataBucket', {
    bucketName: `${config.stageName}-${config.accountId}-${config.region}-orbit-timing-data`,
  ...
  const timingDLQ = new Queue(scope, 'OrbitTimingDLQ', {
    queueName: `${config.stageName}-orbit-timing-dlq`,
    retentionPeriod: Duration.days(14), // Keep failed messages for 14 days
    removalPolicy: isDev ? RemovalPolicy.DESTROY : RemovalPolicy.RETAIN,
  })

  // Create main queue for timing events with DLQ
  const timingQueue = new Queue(scope, 'OrbitTimingQueue', {
    queueName: `${config.stageName}-orbit-timing-queue`,
  ...
  const timingMergerLambda = new NodejsFunction(scope, 'OrbitTimingMerger', {
