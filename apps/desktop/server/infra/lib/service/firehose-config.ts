...
export function createFirehoseStreams(
  ...
  const clientDeliveryStream = new CfnDeliveryStream(
    scope,
    'OrbitClientLogsToOs-client',
    {
      deliveryStreamName: `${config.stageName}-orbit-client-logs`,
  ...
  const serverDeliveryStream = new CfnDeliveryStream(
    scope,
    'OrbitServerLogsToOs-server',
    {
      deliveryStreamName: `${config.stageName}-orbit-server-logs`,
      amazonopensearchserviceDestinationConfiguration: {
        domainArn: config.opensearchDomain.domainArn,
  ...
  const logsToFirehoseRole = new IamRole(scope, 'OrbitLogsToFirehoseRole', {
    assumedBy: new ServicePrincipal('logs.amazonaws.com'),
  })

  const logsToFirehosePolicy = new Policy(
    scope,
    'OrbitLogsToFirehoseWritePolicy',
    {
      statements: [
        new PolicyStatement({
