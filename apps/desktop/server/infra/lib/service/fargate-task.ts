...
export function createFargateTask(
  ...
  const fargateTaskRole = new IamRole(scope, 'OrbitFargateTaskRole', {
  ...
  const taskExecutionRole = new IamRole(scope, 'OrbitTaskExecRole', {
    assumedBy: new ServicePrincipal('ecs-tasks.amazonaws.com'),
    managedPolicies: [
  ...
  const taskDefinition = new FargateTaskDefinition(scope, 'OrbitTaskDefinition', {
  ...
  const containerName = 'OrbitServerContainer'
  ...
  taskDefinition.addContainer(containerName, {
    ...
    environment: {
      ...
      ORBIT_ENV: config.stageName,
      SHOW_ALL_REQUEST_LOGS: 'true',
    },
    logging: new AwsLogDriver({
      streamPrefix: 'orbit-server',
      logGroup: config.serverLogGroup,
    }),
  })

  const taskLogsPolicy = new Policy(scope, 'OrbitTaskLogsPolicy', {
    statements: [
      new PolicyStatement({
        actions: [
          'logs:CreateLogGroup',
        ...
        resources: [
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/orbit/${config.stageName}/client`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/orbit/${config.stageName}/client:log-stream:*`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/orbit/${config.stageName}/server`,
          `arn:aws:logs:${stack.region}:${stack.account}:log-group:/orbit/${config.stageName}/server:log-stream:*`,
        ],
      }),
    ],
  })
