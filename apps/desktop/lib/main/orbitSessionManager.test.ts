...
const mockOrbitStreamController = {
  ...
  setMode: mock(),
  getCurrentMode: mock(() => OrbitMode.TRANSCRIBE),
  scheduleConfigUpdate: mock(() => Promise.resolve()),
  getAudioDurationMs: mock(() => 1000),
  endInteraction: mock(),
  cancelTranscription: mock(),
}
mock.module('./orbitStreamController', () => ({
  orbitStreamController: mockOrbitStreamController,
}))
...
describe('orbitSessionManager', () => {
  beforeEach(() => {
    ...
    Object.values(mockOrbitStreamController).forEach(mockFn => mockFn.mockClear())
    ...
    mockOrbitStreamController.initialize.mockResolvedValue(true)
    mockOrbitStreamController.startGrpcStream.mockResolvedValue({
      response: { transcript: 'test transcript' },
      audioBuffer: Buffer.from('audio-data'),
      sampleRate: 16000,
    })
    mockOrbitStreamController.getAudioDurationMs.mockReturnValue(1000)
  ...
  test('should start session successfully', async () => {
    const { OrbitSessionManager } = await import('./orbitSessionManager')
    const session = new OrbitSessionManager()

    await session.startSession(OrbitMode.TRANSCRIBE)

    expect(mockOrbitStreamController.initialize).toHaveBeenCalledWith(
      OrbitMode.TRANSCRIBE,
    )
    expect(mockOrbitStreamController.startGrpcStream).toHaveBeenCalled()
    expect(mockOrbitStreamController.setMode).toHaveBeenCalledWith(
      OrbitMode.TRANSCRIBE,
    )
    expect(mockVoiceInputService.startAudioRecording).toHaveBeenCalled()
    expect(
      mockRecordingStateNotifier.notifyRecordingStarted,
    ).toHaveBeenCalledWith(OrbitMode.TRANSCRIBE)
  })
  ...
  test('should fetch and send context in background', async () => {
    ...
    expect(mockOrbitStreamController.scheduleConfigUpdate).toHaveBeenCalled()
  })
  ...
  test('should fail to start session when controller fails', async () => {
    mockOrbitStreamController.initialize.mockResolvedValueOnce(false)
  ...
  test('should change mode during session', async () => {
    ...
    session.setMode(OrbitMode.EDIT)

    expect(mockOrbitStreamController.setMode).toHaveBeenCalledWith(OrbitMode.EDIT)
    expect(
      mockRecordingStateNotifier.notifyRecordingStarted,
    ).toHaveBeenCalledWith(OrbitMode.EDIT)
  })
  ...
  test('should cancel session successfully', async () => {
    ...
    expect(mockOrbitStreamController.cancelTranscription).toHaveBeenCalled()
  ...
  test('should complete session with sufficient audio', async () => {
    ...
    mockOrbitStreamController.getAudioDurationMs.mockReturnValue(500)

    await session.startSession(OrbitMode.TRANSCRIBE)
    await session.completeSession()

    expect(mockVoiceInputService.stopAudioRecording).toHaveBeenCalled()
    expect(mockOrbitStreamController.endInteraction).toHaveBeenCalled()
    expect(mockRecordingStateNotifier.notifyRecordingStopped).toHaveBeenCalled()
  })
  ...
  test('should cancel session when audio too short', async () => {
    ...
    mockOrbitStreamController.getAudioDurationMs.mockReturnValue(50)

    await session.startSession(OrbitMode.TRANSCRIBE)
    await session.completeSession()

    expect(mockOrbitStreamController.cancelTranscription).toHaveBeenCalled()
    expect(mockOrbitStreamController.endInteraction).not.toHaveBeenCalled()
    expect(mockRecordingStateNotifier.notifyRecordingStopped).toHaveBeenCalled()
  })

  test('should handle successful transcription response', async () => {
    const mockTranscript = 'Hello world'
    mockOrbitStreamController.startGrpcStream.mockResolvedValueOnce({
  ...
  test('should handle unexpected transcription error', async () => {
    ...
    mockOrbitStreamController.startGrpcStream.mockRejectedValueOnce(error)
  ...
  test('should handle context fetch error gracefully', async () => {
    mockOrbitStreamController.scheduleConfigUpdate.mockRejectedValueOnce(
  ...
  test('should handle complete session flow', async () => {
    ...
    expect(mockOrbitStreamController.initialize).toHaveBeenCalled()
    expect(mockVoiceInputService.startAudioRecording).toHaveBeenCalled()
