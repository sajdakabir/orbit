...
export class OrbitStreamController {
  ...
  public async initialize(mode: OrbitMode): Promise<boolean> {
    ...
    console.log('[OrbitStreamController] Starting new interaction stream.')
  ...
  public async startGrpcStream(): Promise<{
    ...
    if (this.hasStartedGrpc) {
      log.warn('[OrbitStreamController] gRPC stream already started')
      throw new Error('Stream already started')
    }

    console.log('[OrbitStreamController] Starting gRPC stream immediately')
    this.hasStartedGrpc = true
    this.abortController = new AbortController()
    const abortSignal = this.abortController.signal
    const timingEventName =
      this.currentMode === OrbitMode.EDIT
  ...
  public getCurrentMode(): OrbitMode {
    return this.currentMode
  }

  public setMode(mode: OrbitMode) {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[OrbitStreamController] Cannot change mode - no active stream')
      return
    }

    this.currentMode = mode
    console.log(`[OrbitStreamController] Mode changed to ${mode}`)
  ...
  public scheduleConfigUpdate(context: ContextData) {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[OrbitStreamController] Cannot send config - no active stream')
      return
    }

    console.log('[OrbitStreamController] Queueing config update')
    const config = this.buildStreamConfig(context)
    this.configQueue.push(config)
  }

  private sendModeUpdate(mode: OrbitMode) {
    console.log(`[OrbitStreamController] Sending mode update: ${mode}`)
  ...
  public endInteraction() {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[OrbitStreamController] No active stream to end')
      return
    }

    console.log('[OrbitStreamController] Ending interaction stream')
    this.stopStreaming()
  }

  public cancelTranscription() {
    if (!this.audioStreamManager.isCurrentlyStreaming()) {
      log.warn('[OrbitStreamController] No active stream to cancel')
      return
    }

    console.log('[OrbitStreamController] Cancelling transcription')
  ...
  private async *createStreamGenerator(): AsyncGenerator<TranscribeStreamRequest> {
    console.log(
      '[OrbitStreamController] Starting stream generator (audio-first mode)',
    )
    ...
    for await (const audioChunk of this.audioStreamManager.streamAudioChunks()) {
      if (this.isCancelled) {
        console.log(
          '[OrbitStreamController] Stream cancelled, stopping generator',
        )
        break
      }
      ...
      while (this.configQueue.length > 0) {
        ...
        console.log('[OrbitStreamController] Sending config update from queue')
        yield configMessage
      }
    ...
    while (this.configQueue.length > 0) {
      const configMessage = this.configQueue.shift()!
      console.log(
        '[OrbitStreamController] Sending final config update from queue',
      )
      yield configMessage
    }
  }
...
export const orbitStreamController = new OrbitStreamController()
