...
type InsertTodayEntry = {
  date: string
  content: string
  user_id: string
}

export class TodayTable {
  static async createOrUpdate(
    entryData: InsertTodayEntry,
  ): Promise<TodayEntry> {
    // Check if entry exists for this date and user
    const existing = await this.findByDate(entryData.date, entryData.user_id)

    if (existing) {
      // Update existing entry
      await this.updateContent(existing.id, entryData.content)
      return {
        ...existing,
    ...
    const query = `
      INSERT INTO today (id, date, content, user_id, created_at, updated_at, deleted_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `
    const params = [
      newEntry.id,
  ...
  static async findByDate(
    ...
    const entry = await get<TodayEntry>(
      'SELECT * FROM today WHERE date = ? AND user_id = ? AND deleted_at IS NULL',
      [date, userId],
    )
    return entry || null
  }

  static async findById(id: string): Promise<TodayEntry | undefined> {
    return await get<TodayEntry>('SELECT * FROM today WHERE id = ?', [id])
  }

  static async findAll(user_id?: string): Promise<TodayEntry[]> {
    const query = user_id
      ? 'SELECT * FROM today WHERE user_id = ? AND deleted_at IS NULL ORDER BY date DESC'
      : 'SELECT * FROM today WHERE user_id IS NULL AND deleted_at IS NULL ORDER BY date DESC'
    const params = user_id ? [user_id] : []
    return await all<TodayEntry>(query, params)
  }

  static async updateContent(id: string, content: string): Promise<void> {
    const query = 'UPDATE today SET content = ?, updated_at = ? WHERE id = ?'
    await run(query, [content, new Date().toISOString(), id])
  }

  static async softDelete(id: string): Promise<void> {
    const query = 'UPDATE today SET deleted_at = ?, updated_at = ? WHERE id = ?'
    await run(query, [new Date().toISOString(), new Date().toISOString(), id])
  }

  static async deleteAllUserData(userId: string): Promise<void> {
    const query =
      'UPDATE today SET deleted_at = ?, updated_at = ? WHERE user_id = ?'
    await run(query, [
      new Date().toISOString(),
      new Date().toISOString(),
      userId,
...
type UserMetadataRow = {
  ...
  onboarding_completed: number // SQLite stores boolean as 0 or 1
...
export class UserMetadataTable {
  static async insert(metadataData: InsertUserMetadata): Promise<UserMetadata> {
    ...
    const params = [
      ...
      newMetadata.onboarding_completed ? 1 : 0,
      newMetadata.onboarding_step,
  ...
  static async update(
    ...
    if (updates.onboarding_completed !== undefined) {
      ...
      params.push(updates.onboarding_completed ? 1 : 0)
    }
    if (updates.onboarding_step !== undefined) {
      fields.push('onboarding_step = ?')
      params.push(updates.onboarding_step)
    }
  ...
  static async upsert(metadata: UserMetadata): Promise<void> {
    ...
    const params = [
      ...
      metadata.onboarding_completed ? 1 : 0,
      metadata.onboarding_step,
