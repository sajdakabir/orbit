...
export class VoiceInputService {
   ...
   * Does NOT start the OrbitStreamController - that should be done separately.
   */
  ...
  public stopAudioRecording = async () => {
    ...
    try {
      await (audioRecorderService as any).awaitDrainComplete?.(150)
  ...
  public setUpAudioRecorderListeners = () => {
    // Note: audio-chunk and audio-config are now handled directly by OrbitStreamController
