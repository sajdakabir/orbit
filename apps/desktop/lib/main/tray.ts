...
function getTrayIconPath(): string {
  ...
  if (!app.isPackaged) {
    // Use the app asset in dev so we can iterate quickly
    return join(__dirname, '../../app/assets/orbit.png')
  }
  // Use the transparent tray icon, not the full app icon
  return join(process.resourcesPath, 'build', 'trayTemplate.png')
}
...
export async function createAppTray(): Promise<void> {
  ...
  // Mark as template image so macOS adapts it for light/dark menu bar
