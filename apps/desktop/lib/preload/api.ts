...
const api = {
  ...
  today: {
    getAll: () => ipcRenderer.invoke('today:get-all'),
    getByDate: (date: string, userId: string) =>
      ipcRenderer.invoke('today:get-by-date', { date, userId }),
    createOrUpdate: (entry: any) =>
      ipcRenderer.invoke('today:create-or-update', entry),
    delete: (id: string) => ipcRenderer.invoke('today:delete', id),
  },
  dictionary: {
  ...
  // Electron store (synchronous local cache)
  electronStoreGet: (key: string) => ipcRenderer.sendSync('electron-store-get', key),
  ...
  composio: {
    getApps: () => ipcRenderer.invoke('composio:get-apps'),
    getConnections: () => ipcRenderer.invoke('composio:get-connections'),
    initiateConnection: (appName: string) =>
      ipcRenderer.invoke('composio:initiate-connection', appName),
    ...
    ) => ipcRenderer.invoke('composio:execute-tool', toolSlug, userId, args),
    getTools: (toolkitSlug: string) =>
      ipcRenderer.invoke('composio:get-tools', toolkitSlug),
    fetchInbox: () => ipcRenderer.invoke('composio:fetch-inbox'),
  },

  // Action execution (voice command → Composio tool)
  action: {
    confirm: (actionId: string, editedParameters?: Record<string, any>) =>
      ipcRenderer.send('action-confirm', { actionId, editedParameters }),
    cancel: (actionId: string) =>
      ipcRenderer.send('action-cancel', { actionId }),
    disambiguate: (actionId: string, field: string, selectedValue: string) =>
      ipcRenderer.send('action-disambiguate', { actionId, field, selectedValue }),
    connectTool: (toolkitSlug: string) =>
      ipcRenderer.send('action-connect-tool', toolkitSlug),
  },
}
