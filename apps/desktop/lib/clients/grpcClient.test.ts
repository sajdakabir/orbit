...
mock.module('@/app/generated/orbit_pb', () => ({
  OrbitService: { typeName: 'OrbitService' },
...
describe('GrpcClient Business Logic Tests', () => {
  ...
  describe('Transcription Stream Business Logic', () => {
    ...
    test('should include metadata in transcription', async () => {
      ...
      await grpcClient.transcribeStream(audioStream, OrbitMode.TRANSCRIBE)
    ...
    test('should handle transcription errors gracefully', async () => {
      ...
      expect(
        grpcClient.transcribeStream(audioStream, OrbitMode.TRANSCRIBE),
