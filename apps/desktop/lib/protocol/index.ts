...
const PROTOCOL = ORBIT_ENV === 'prod' ? 'orbit' : `orbit-dev`
...
function handleProtocolUrl(url: string) {
  try {
    ...
    if (
      ...
      // Better Auth OAuth callback - session token passed from server via custom protocol
      const sessionToken = urlObj.searchParams.get('token')
      ...
      if (
        ...
        console.warn('No session token found in protocol URL or no main window')
      }
    } else if (
      urlObj.protocol === `${PROTOCOL}:` &&
      urlObj.hostname === 'auth' &&
      ...
      urlObj.protocol === `${PROTOCOL}:` &&
      urlObj.hostname === 'composio'
    ) {
      const status = urlObj.searchParams.get('status') || ''
      const connectedAccountId =
        urlObj.searchParams.get('connected_account_id') || ''

      console.log(
        `[Protocol] Composio callback: status=${status}, id=${connectedAccountId}`,
      )

      // Invalidate context cache since a new connection was added/changed
      actionContextProvider.invalidateCache()

      // Resume the pending action if the connection was successful
      if (status === 'success') {
        actionExecutionService.retryAfterConnection()
      }
