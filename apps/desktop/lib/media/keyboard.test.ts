...
const mockOrbitSessionManager = {
  startSession: mock(),
  completeSession: mock(),
  setMode: mock(),
  cancelSession: mock(),
}
mock.module('../main/orbitSessionManager', () => ({
  orbitSessionManager: mockOrbitSessionManager,
}))
...
describe('Keyboard Module', () => {
  beforeEach(async () => {
    ...
    mockOrbitSessionManager.startSession.mockClear()
    mockOrbitSessionManager.completeSession.mockClear()
    mockOrbitSessionManager.setMode.mockClear()
    mockOrbitSessionManager.cancelSession.mockClear()
  ...
  describe('Shortcut Detection Business Logic', () => {
    test('should activate shortcut when keys match', async () => {
      ...
      expect(mockOrbitSessionManager.startSession).toHaveBeenCalled()
    ...
    test('should deactivate shortcut when keys are released', async () => {
      ...
      expect(mockOrbitSessionManager.completeSession).toHaveBeenCalled()
      expect(console.info).toHaveBeenCalledWith(
    ...
    test('should not activate shortcut when globally disabled', async () => {
      ...
      expect(mockOrbitSessionManager.startSession).not.toHaveBeenCalled()
    })
    ...
    test('should require exact key match for shortcut activation', async () => {
      ...
      expect(mockOrbitSessionManager.startSession).toHaveBeenCalledWith(
    ...
    test('should allow repeated shortcut activations with exact matching', async () => {
      ...
      expect(mockOrbitSessionManager.startSession).toHaveBeenCalledTimes(1)
      ...
      expect(mockOrbitSessionManager.completeSession).toHaveBeenCalledTimes(1)
