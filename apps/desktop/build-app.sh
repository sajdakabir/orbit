...
# Command-line env takes precedence over .env
if [ -n "$_CLI_ORBIT_ENV" ]; then
  export ORBIT_ENV="$_CLI_ORBIT_ENV"
fi
...
create_dmg() {
    ...
    # Determine stage and handle notarization
    local stage="${ORBIT_ENV:-prod}"
    if [ "$stage" = "prod" ]; then
      if [ -z "$APPLE_ID" ] || [ -z "$APPLE_APP_SPECIFIC_PASSWORD" ] || [ -z "$APPLE_TEAM_ID" ]; then
        print_warning "No notarization credentials found. Building prod without notarization/code signing."
    ...
    if [ -z "${VITE_ORBIT_ENV}" ]; then
      export VITE_ORBIT_ENV="${ORBIT_ENV:-dev}"
      print_info "Set VITE_ORBIT_ENV=${VITE_ORBIT_ENV} for build-time embedding"
    fi
    bun run electron-vite build
    ...
    if [ -d "dist" ]; then
        ...
        local dmg_name
        if [ "$stage" = "prod" ]; then
          dmg_name="Orbit-Installer.dmg"
        else
          dmg_name="Orbit-${stage}-Installer.dmg"
        fi
...
create_windows_installer() {
    ...
    docker run --rm --platform linux/amd64 \
      ...
      --env VITE_ORBIT_VERSION="${VITE_ORBIT_VERSION}" \
      --env ORBIT_ENV="${ORBIT_ENV}" \
      ...
      bash -c "
        ...
        exe_path=\$(ls -t dist/Orbit*.exe 2>/dev/null | head -n 1)
        if [ -n \"\$exe_path\" ]; then
          dest_name=\$([ \"\${ORBIT_ENV:-dev}\" = \"prod\" ] && echo \"Orbit-Installer.exe\" || echo \"Orbit-\${ORBIT_ENV}-Installer.exe\")
...
main() {
    ...
    print_status "Starting Orbit $PLATFORM build process..."
    echo
    ...
    case $PLATFORM in
        "mac")
            ...
            if [ -z "${ORBIT_ENV}" ] || [ "${ORBIT_ENV}" = "prod" ]; then
              print_info "Your DMG installer is ready: dist/Orbit-Installer.dmg"
            else
              print_info "Your DMG installer is ready: dist/Orbit-${ORBIT_ENV}-Installer.dmg"
            fi
            ;;
        "windows")
            create_windows_installer
